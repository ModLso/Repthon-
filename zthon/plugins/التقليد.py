...
async def echo(event):
    ...
    Repthonevent = await edit_or_reply(event, "⌔∮ يتم تفعيل هذا الامر انتظر قليلا ،")

    user, rank = await get_user_from_event(event, Repthonevent, nogroup=True)
...
@Repthon.ar_cmd(pattern="الغاء تقليد$")
...
async def echo(event):
    ...
    if is_echo(chat_id, user_id):
        ...
        try:
            ...
            await edit_delete(Repthonevent, f"**خطأ:**\n`{e}`")
...
@Repthon.ar_cmd(pattern="حذف المقلدهم( للكل)?")
...
@Repthon.ar_cmd(pattern="المقلدهم( للكل)?$")
...
@Repthon.ar_cmd(incoming=True, edited=False)
